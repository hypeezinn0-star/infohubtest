...
TabUtil:CreateDropdown({
    ...
    Options = temasRayfield,
    CurrentOption = { "Default" },
    Flag = "util_tema_rayfield",
    Callback = function(opcao)
        local tema = type(opcao) == "table" and opcao[1] or opcao
        local ok = pcall(function()
            Window:ModifyTheme(tema)
        end)
        if ok then
            notificar("Tema", "Tema aplicado: " .. tostring(tema) .. ".", 2, "palette")
        else
            notificar("Tema", "Este tema não está disponível nesta versão da Rayfield.", 3, "alert-triangle")
        end
    end,
})
