...
-- ======== ABA: MOVIMENTO (self: FOV / WalkSpeed / JumpPower) ========
local TabMov = Window:CreateTab("Movimento", "move")

-- estado desejado pelo usuário (reaplicado ao renascer)
local movConfig = {
    walkspeed = 16,   -- padrão do Roblox
    jumppower = 50,   -- padrão do Roblox
    walkOn = false,
    jumpOn = false,
}
...
local function aplicarMovimento()
    ...
    pcall(function()
        if movConfig.walkOn then
            hum.WalkSpeed = movConfig.walkspeed
        end
        if movConfig.jumpOn then
            -- alguns jogos usam JumpHeight em vez de JumpPower
            hum.UseJumpPower = true
            hum.JumpPower = movConfig.jumppower
        end
    end)
end

-- reaplica sempre que o personagem renascer
...
TabMov:CreateSlider({
    ...
    Callback = function(valor)
        pcall(function()
            ...
            if cam then cam.FieldOfView = valor end
        end)
    end
})
...
TabMov:CreateToggle({
    ...
    Flag = "mov_walk_on",
    Callback = function(valor)
        movConfig.walkOn = valor
        local hum = getHumanoid()
        if valor then
            aplicarMovimento()
            notificar("WalkSpeed", "Velocidade personalizada ativada.", 3, "footprints")
        elseif hum then
            pcall(function() hum.WalkSpeed = 16 end)
            notificar("WalkSpeed", "Velocidade restaurada (16).", 3, "footprints")
        end
    end
})
...
TabMov:CreateSlider({
    ...
    Flag = "mov_walk_val",
    Callback = function(valor)
        movConfig.walkspeed = valor
        if movConfig.walkOn then aplicarMovimento() end
    end
})
...
TabMov:CreateToggle({
    ...
    Flag = "mov_jump_on",
    Callback = function(valor)
        movConfig.jumpOn = valor
        local hum = getHumanoid()
        if valor then
            aplicarMovimento()
            notificar("JumpPower", "Pulo personalizado ativado.", 3, "chevrons-up")
        elseif hum then
            pcall(function() hum.JumpPower = 50 end)
            notificar("JumpPower", "Pulo restaurado (50).", 3, "chevrons-up")
        end
    end
})

TabMov:CreateSlider({
    Name = "Valor JumpPower",
    Range = { 50, 500 },
    Increment = 5,
    CurrentValue = 50,
    Flag = "mov_jump_val",
    Callback = function(valor)
        movConfig.jumppower = valor
        if movConfig.jumpOn then aplicarMovimento() end
    end
})
...
-- ---- SERVER RANDOM (SERVIDOR ALEATÓRIO) ----
TabUtil:CreateButton({
    ...
    Callback = function()
        ...
        task.spawn(function()
            ...
            for _, sv in ipairs(dados.data) do
                if sv.id ~= game.JobId and sv.playing and sv.maxPlayers
                    and sv.playing < sv.maxPlayers then
                    candidatos[#candidatos + 1] = sv
                end
            end

            if #candidatos == 0 then
                notificar("Servidor Aleatório", "Nenhum servidor disponível.", 4, "info")
                return
            end
...
TabUtil:CreateButton({
    ...
    Callback = function()
        ...
        for _, nome in ipairs({ "InfoHubIntro", "InfoHubErro" }) do
