...
local function gen2ResolverIcon(icon)
    ...
    if string.sub(icon, 1, 11) == "rbxassetid://"
        or string.sub(icon, 1, 10) == "rbxasset://"
        or string.sub(icon, 1, 10) == "rbxthumb://" then
        return icon
    end
...
local avCabecalho = TabAvatares:CreateParagraph({
    Title = "Jogadores",
    Content = "Carregando thumbnails dos jogadores...",
})
...
local function atualizarAvatares()
    ...
    pcall(function()
        avCabecalho:Set({
            ...
            Content = "Cada jogador possui seu thumbnail ao lado do nome.",
        })
    end)
end
...
local function obterInicioServidor()
    for _, nomeAtributo in ipairs({ "ServerCreatedAt", "ServerStartTime", "ServerCreationTime" }) do
        local valor = game:GetAttribute(nomeAtributo)
        if type(valor) == "number" and valor > 0 then return valor end
        if type(valor) == "string" then
            local convertido = tonumber(valor)
            if convertido and convertido > 0 then return convertido end
        end
    end
    return nil
end

local inicioServidorUnix = obterInicioServidor()
local criacaoServidorTexto = inicioServidorUnix
    and os.date("%H:%M:%S - %d/%m/%Y", inicioServidorUnix)
    or "Não exposto pela API deste jogo"
TabServidor:CreateParagraph({
    Title = "Criação do servidor",
    Content = criacaoServidorTexto,
})
